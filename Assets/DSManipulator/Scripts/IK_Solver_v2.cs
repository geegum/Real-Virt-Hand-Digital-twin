...
//solver1과 다르게 시간제한을 둚
public class IK_Solver_v2 : MonoBehaviour
{
    ...
    private Coroutine resolvingCoroutine; // 특이점 해결을 위한 Coroutine
    ...
    void Start()
    {
        ...
        minAngle[4] = -135f;
        maxAngle[4] = 135f;
        minAngle[5] = 0f; // joint[5] 고정
        maxAngle[5] = 0f; // joint[5] 고정
        ...
        minAngleVelocity[5] = 0f; // joint[5] 고정
        maxAngleVelocity[5] = 0f; // joint[5] 고정
    ...
    void FollowPath()
    {
        ...
        for (int iteration = 0; iteration < maxIterations; iteration++)
        {
            ...
            for (int i = joint.Length - 2; i >= 0; i--) // joint[5] 제외
            {
                ...
                if (IsSingularity(i))
                {
                    if (resolvingCoroutine != null)
                        StopCoroutine(resolvingCoroutine);

                    resolvingCoroutine = StartCoroutine(ResolveSingularity(i, targetPosition)); // 특이점 해결을 시도
                }
            }
    ...
    IEnumerator ResolveSingularity(int jointIndex, Vector3 targetPosition)
    {
        // 10초 내에 계산이 완료되지 않으면 중지
        ...
        float searchRange = 5f; // 각도 조정 범위 (degrees)
        ...
        for (float angle = currentAngle - searchRange; angle <= currentAngle + searchRange; angle += stepSize)
        {
            ...
            {
                Debug.Log("Singularity resolution timed out.");
                break;
            }

            // joint 회전을 임시로 테스트하여 에러 계산
            Quaternion testRotation = joint[jointIndex].transform.localRotation;
            joint[jointIndex].transform.localRotation = Quaternion.AngleAxis(angle, vangle[jointIndex]);
        ...
        while (Quaternion.Angle(joint[jointIndex].transform.localRotation, optimalRotation) > 0.01f)
        {
        ...
        previousAngles[jointIndex] = optimalAngle;
